...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def contact_access_payment(self, request):
        ...
        # Trigger M-Pesa Charge (STK Push)
        data = {
            "email": email,
        ...
        # Use Session to ignore local proxy env vars that might be breaking it
        session = requests.Session()
        session.trust_env = False
        
        # Add robust headers
        headers.update({
            "User-Agent": "HouseHunt/1.0",
            "Connection": "close"
        })
        
        print(f"Initiating Paystack Request to {url} with phone {phone}")
        try:
            # Add timeout to prevent server hanging forever
            response = session.post(url, json=data, headers=headers, timeout=25)
            print(f"Paystack responded with status: {response.status_code}")
            
            res_data = response.json()
            if response.status_code == 200 and res_data.get('status'):
                return Response(res_data['data'])  # returns reference/status
